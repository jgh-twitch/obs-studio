...
#define HAPPY_EYEBALLS_DELAY_MS 200 /* this is the same default as libcurl */
#define HAPPY_EYEBALLS_MAX_ATTEMPTS 6
#define HAPPY_EYEBALLS_CONNECTION_TIMEOUT_MS \
	25000 // Total time to wait for sockets, or all finish trying, whichever is shorter
...
struct happy_eyeballs_ctx {
	/**
	 * socket_fd will be non-zero upon successful connection to the host and port specified.
	 */
	SOCKET socket_fd;

	/**
	 * winner_addr will be non-zero upon successful connection to the host and port specified.
	 */
	struct sockaddr winner_addr;

	/**
	 * winner_addr_len will be non-zero upon successful connection to the host and port specified.
	 */
	...
	/**
	 *  Protects against multiple simultaneous winners writing socket_fd, winner_addr, and winner_addr_len
	 */
	...
	/**
	 *  Event that signals completion of the race, either via winner or error
	 */
	...
	/**
	 *  Domain name resolution time, in nanoseconds (0 until resolution success)
	 */
	...
	/**
	 *  Indicates whether we are in the initial phase of dispatching connections, 
	 *  or if we are waiting for things to connect or time out.
	 */
...
static int check_comodo(struct happy_eyeballs_ctx *context)
{
	...
	if (!h && GetLastError() == WSAHOST_NOT_FOUND) {
		...
		context->error_message =
			"happy-eyeballs: Connection test failed. This error is likely caused by Comodo Internet Security running OBS in sandbox mode. Please add OBS to the Comodo automatic sandbox exclusion list, restart OBS and try again (11001).";
			   ...
			   struct happy_eyeballs_ctx *context)
{
	...
	hints.ai_addrlen = context->bind_addr_len;

	if (hints.ai_addrlen == sizeof(struct sockaddr_in))
		hints.ai_family = AF_INET;
	else if (hints.ai_addrlen == sizeof(struct sockaddr_in6))
...
static void *happy_connect_worker(void *arg)
{
	...
	if (connect(args->sockfd, args->address->ai_addr,
		...
		if (os_event_try(context->race_completed_event) == EAGAIN) {
			...
			context->winner_addr_len = args->address->ai_addrlen;
			signal_end(context);
		}
...
failure:
	...
	// connection candidates may still be getting dispatched, treat as if there's
	// an active candidate
	bool active = os_atomic_load_bool(&context->is_starting);

	// check if we are the last worker running. If so, we'll set the error status
	// and signal the completion event.
	pthread_mutex_lock(&context->candidate_mutex);
	...
	if (active || context->error != 0)
		// we're not last or there is already an error on the context, let's exit.
		goto success;
			 ...
			 struct addrinfo *addr)
{
	...
	if (args == NULL) {
		...
		context->error_message =
			"happy-eyeballs: Failed to allocate memory for worker context";
		pthread_mutex_unlock(&context->candidate_mutex);
		return STATUS_FAILURE;
	}
	...
	if (result != 0) {
		...
		context->error_message =
			"happy-eyeballs: Failed to initialize socket completed event";
		free(args);
		pthread_mutex_unlock(&context->candidate_mutex);
		return STATUS_FAILURE;
	}
	...
	pthread_mutex_unlock(&context->candidate_mutex);
	// Launch worker thread; `args` ownership is passed to the thread here.
...
int happy_eyeballs_create(struct happy_eyeballs_ctx **context)
{
	...
	// race_completed_event will be signalled when there is a winner or all attempts have failed
	int result =
		os_event_init(&ctx->race_completed_event, OS_EVENT_TYPE_MANUAL);

	// this mutex is used to avoid the situation where we may have
	// two simultaneous winners and inconsistent values set to the context.
	if (result == 0)
		result = pthread_mutex_init(&ctx->winner_mutex, NULL);
	bool have_winner_mutex = result == 0;

	// this mutex is used to avoid the situation where we may be mutating the candidate
	// array while iterating it.
	if (result == 0)
	...
	*context = NULL;
	...
	// Error codes from pthread_mutex_init are positive, os_event_init is negative.
	// We have promised to return negative error codes, so we are making them all
	// negative here.
	return -abs(result);
}
			   ...
			   const char *hostname, int port)
{
	...
	//	2. There are no more candidates in the linked list (ie. `next` is null)
	//	3. We have seen two candidates of the same family in a row, stop happy
	//	   eyeballs and let the previous attempt go it alone.
	for (int i = 0; i < HAPPY_EYEBALLS_MAX_ATTEMPTS && next &&
		...
		// Wait until the delay between attempts times out or we get signalled...
		...
		if (result == 0)
			...
			context->error = result;
			context->error_message =
				"happy-eyeballs: Encountered error waiting for race_completed_event";
			return STATUS_FAILURE;
		}
...
#endif
	// We do not need to lock context->candidate_mutex here because the candidate array is _only_ mutated
	// in happy_eyeballs_connect. Using this function at the same time as connect is a programmer error.
	// Locking it here will cause a deadlock with join.

	// Shutdown non-winning sockets (but keep the socket alive so that things error out in threads)
	for (size_t i = 0; i < context->candidates.num; i++) {
				   ...
				   struct sockaddr *addr)
{
	...
	if (context->winner_addr_len > 0)
		memcpy(addr, &context->winner_addr, sizeof(struct sockaddr));
	...
	const struct happy_eyeballs_ctx *context)
{
	...
	return context->name_resolution_time_ns;
}
